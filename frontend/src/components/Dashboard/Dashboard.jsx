...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        const [historyRes, limitsRes] = await Promise.all([getSessionHistory(), getMyLimits()]);
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4 text-center transition-colors">
            ...
            <div className="text-xs text-gray-400 dark:text-gray-500 mt-1">{t('dashboard.longestStreak')}: {user?.longestStreak || 0}</div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-4 text-center transition-colors">
            ...
            <div className="text-xs text-gray-400 dark:text-gray-500 mt-1">{user?.totalSessions || 0} {t('dashboard.sessions')}</div>
          </div>
        </div>
        ...
        {!isAdmin && limits && (
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-5 mb-8 transition-colors">
            <h2 className="text-base font-bold text-gray-800 dark:text-white mb-4">⚡ {t('dashboard.dailyLimit')}</h2>
            <LimitBar label={`📚 ${t('vocab.title')}`} used={limits.vocabulary.used} limit={limits.vocabulary.limit} color="bg-green-500" />
            <LimitBar label={`🎤 ${t('speaking.title')}`} used={limits.speaking.used} limit={limits.speaking.limit} color="bg-blue-500" />
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">🔄 {t('dashboard.resetsAt')}</p>
          </div>
        )}

        {/* Action cards */}
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          <Link to="/vocabulary" className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 hover:shadow-lg transition-all border-l-4 border-green-500">
          ...
          <Link to="/speaking" className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 hover:shadow-lg transition-all border-l-4 border-blue-500">
            <div className="text-4xl mb-3">🎤</div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-5 mb-8 transition-colors">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base font-bold text-gray-800 dark:text-white">🏆 {t('dashboard.leaderboard')}</h2>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`font-semibold truncate ${isMe ? 'text-blue-700 dark:text-blue-300' : 'text-gray-800 dark:text-white'}`}>
                        {entry.name} {isMe && <span className="text-xs font-normal">({t('dashboard.youLabel')})</span>}
                      </p>
        ...
        {/* Session history */}
        <div>
          <h2 className="text-xl font-bold text-gray-800 dark:text-white mb-4">📋 {t('dashboard.recentSessions')}</h2>
          {loadingHistory ? (
            ...
            <div className="space-y-3">
              {history.map((s) => (
                <div key={s._id} className="bg-white dark:bg-gray-800 rounded-xl shadow p-4 flex items-center justify-between transition-colors">
                  <div>
